        ...
        body: Stack(children: [
          SafeArea(
            child: CustomScrollView(slivers: [
              ...
              SliverToBoxAdapter(child: _section('NOW', '24h', const [
                _NowCard(name: 'Aki', area: '心斎橋周辺', text: 'この辺でカフェ探してる ☕'),
                _NowCard(name: 'Sora', area: '難波周辺', text: '今からちょっとゲームする'),
              ])),
              ...
              child: IgnorePointer(
                child: Container(
                  ...
                  decoration: const BoxDecoration(
                    ...
                    gradient: RadialGradient(colors: [Color(0x4D9BA6FF), Color(0x169BA6FF), Colors.transparent]),
                  ),
                ),
              ),
            ),
  ...
  Widget _top() => Padding(
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      ...
      Container(
        ...
        decoration: BoxDecoration(color: Colors.white10, borderRadius: BorderRadius.circular(99), border: Border.all(color: Colors.white12)),
        child: const Text('10分圏 • Live', style: TextStyle(color: Colors.white60, fontSize: 12)),
      ),
    ]),
  );

  Widget _hero() => const Padding(
  ...
  Widget _userCard(NearUser user, {bool nearest = false}) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        color: nearest ? const Color(0xFF151824) : const Color(0xFF10121A),
        borderRadius: BorderRadius.circular(23),
        border: Border.all(color: nearest ? const Color(0x52626CFF) : const Color(0x2529333C)),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFF202640), borderRadius: BorderRadius.circular(16)),
            child: Text(user.name.substring(0, 1), style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
          ),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('${user.name} ${user.verified ? '✓' : ''}', style: const TextStyle(fontWeight: FontWeight.w900)),
            const SizedBox(height: 5),
            Wrap(spacing: 5, children: user.interests.map((x) => Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
              decoration: BoxDecoration(color: Colors.white10, borderRadius: BorderRadius.circular(99)),
              child: Text(x, style: const TextStyle(fontSize: 11, color: Colors.white60)),
            )).toList()),
          ])),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text(user.activity, style: const TextStyle(color: Color(0xFF67F7C4), fontSize: 11, fontWeight: FontWeight.w800)),
            const SizedBox(height: 4),
            Text('${user.distance}m', style: const TextStyle(color: Colors.white54, fontSize: 11)),
          ]),
        ]),
        if (user.recent.isNotEmpty)
          Padding(padding: const EdgeInsets.only(top: 11), child: Text(user.recent, style: const TextStyle(color: Colors.white54, fontSize: 12))),
...
class _NowCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    margin: const EdgeInsets.only(bottom: 10),
    padding: const EdgeInsets.all(15),
    decoration: BoxDecoration(color: const Color(0xFF111722), borderRadius: BorderRadius.circular(23), border: Border.all(color: Colors.white10)),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Container(width: 42, height: 42, alignment: Alignment.center, decoration: BoxDecoration(color: const Color(0xFF202640), borderRadius: BorderRadius.circular(14)), child: Text(name.substring(0, 1), style: const TextStyle(fontWeight: FontWeight.w900))),
        const SizedBox(width: 10),
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(name, style: const TextStyle(fontWeight: FontWeight.w900)), Text(area, style: const TextStyle(color: Colors.white45, fontSize: 11))]),
      ]),
      const SizedBox(height: 10),
      Text(text, style: const TextStyle(color: Colors.white70)),
      const SizedBox(height: 8),
      const Text('今この街で見る • 24h', style: TextStyle(color: Colors.white38, fontSize: 11)),
    ]),
  );
}
...
class _NearChatState extends State<NearChat> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(widget.user.name, style: const TextStyle(fontWeight: FontWeight.w900)), const Text('● JUST NOW', style: TextStyle(color: Color(0xFF67F7C4), fontSize: 11))])),
    body: Column(children: [
      Expanded(child: ListView(padding: const EdgeInsets.all(14), children: [
        for (final m in messages)
          Align(
            alignment: m.startsWith('近くにいる') ? Alignment.centerRight : Alignment.centerLeft,
            child: Container(
              margin: const EdgeInsets.symmetric(vertical: 5),
              padding: const EdgeInsets.symmetric(horizontal: 13, vertical: 11),
              decoration: BoxDecoration(color: m.startsWith('近くにいる') ? const Color(0xFFECEFFF) : const Color(0xFF141824), borderRadius: BorderRadius.circular(18)),
              child: Text(m, style: TextStyle(color: m.startsWith('近くにいる') ? Colors.black : Colors.white)),
            ),
          ),
      ])),
      SafeArea(child: Padding(padding: const EdgeInsets.all(10), child: Row(children: [
        Expanded(child: TextField(controller: controller, decoration: InputDecoration(hintText: 'メッセージ…', filled: true, fillColor: const Color(0xFF10131B), border: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: BorderSide.none)))),
